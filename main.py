...
class TextVoiceLangSplit(Star):
    ...
    async def _translate_text(self, text: str, event: AstrMessageEvent) -> str | None:
        ...
        prompt = (
            ...
            f"5. LIMIT TAG STACKING: Never place a high-intensity emotion tag "
            f"and a physical action tag back-to-back "
            f"(e.g., NEVER write [angry] [shouting] or [sad] [sobbing]). "
            f"Keep it to a maximum of 2 tags per sentence, "
            f"and ensure they do not conflict.\n"
            f"6. Custom free-form descriptions are supported "
            f"but must remain moderate: "
            f"[whispers gently], [dramatic tone], [laughing nervously].\n"
            ...
            f"8. SAFETY & STABILITY: Avoid using volatile physical vocalizations "
            f"like [screaming], [shouting], [groaning], or [moaning] "
            f"as they cause severe audio generation glitches. "
            f"To express high energy or extreme anger, "
            f"use intensity modifiers with safer emotions "
            f"(e.g., [extremely angry], [very upset], [loud]) "
            f"instead of physical action tags.\n\n"
            f"=== BASIC EMOTIONS ===\n"
