...
class TextVoiceLangSplit(Star):
    ...
    async def _translate_text(self, text: str, event: AstrMessageEvent) -> str | None:
        ...
        prompt = (
            ...
            f"(e.g. [laughing], [pause]) may appear anywhere in the text.\n"
            f"3. Assign at most ONE emotion tag per sentence. "
            f"If a sentence has no clear emotion, omit the tag.\n"
            f"4. STRICTLY NO BACK-TO-BACK STACKING: Never place two tags "
            f"immediately next to each other — "
            f"no emotion+emotion, no emotion+audio-effect, "
            f"no audio-effect+audio-effect. "
            f"Multiple tags stacked at the same position cause audio artifacts "
            f"and unpredictable glitches. Only one tag per position. "
            f"(wrong: [angry] [frustrated] text, [scared] [gasp] text; "
            f"correct: pick the single most fitting tag).\n"
            f"5. NO REDUNDANT ONOMATOPOEIA: If you use a physical "
            f"vocalization tag (like [gasp], [sighing], [laughing], [panting]), "
            f"you MUST NOT follow it with written phonetic vocalization words "
            f"or onomatopoeia "
            f"(wrong: [gasp] ふぅ… or [sighing] はぁ…; "
            f"correct: either keep the tag alone and start with normal words "
            f"like [gasp] びっくりした, "
            f"or omit the tag and just write the words "
            f"like ふぅ… びっくりした).\n"
            f"6. ALWAYS FOLLOW WITH NORMAL TEXT: According to Fish Audio official "
            f"guidelines, always \"Add appropriate text after sound tags\". "
            f"Do not let a sound tag stand alone "
            f"or be followed only by punctuation or ellipses "
            f"(wrong: [gasp] …; correct: [gasp] What is that?).\n"
            f"7. EMOTION TAG SPACING (PREVENT COLLAPSE): Never change emotion "
            f"tags rapidly across short clauses or short words. Each emotion tag "
            f"must control at least one full, normal sentence. "
            f"DO NOT write: [sad] うぅ… [frustrated] なんでまた… "
            f"(WRONG — switching emotions within 2-3 syllables "
            f"causes immediate audio collapse). "
            f"Instead, unify closely-related moods under "
            f"a single compatible emotion tag: "
            f"[frustrated] うぅ… なんでまた… (CORRECT).\n"
            f"8. NO WEAK VOWEL INITIALS UNDER TAGS (PREVENT INITIAL EXPLOSION): "
            f"Do not place any emotion or vocalization tags immediately before "
            f"short, isolated vowel-only onomatopoeias "
            f"(like うぅ…, あっ…, えっ…, おぉ…) "
            f"at the very beginning of the audio. "
            f"TTS models need strong phonetic consonants to stabilize at the start. "
            f"If the translation starts with a sigh/groan, "
            f"either omit the emotion tag for that segment, "
            f"or merge it into the next sentence.\n"
            f"9. KEEP IT EXTREMELY SIMPLE: Do not overuse tags in short text. "
            f"Only add tags when there is a significant, clear emotional shift. "
            f"Space out emotional changes to allow the model to transition smoothly.\n\n"
            f"=== BASIC EMOTIONS ===\n"
            ...
            f"[disdainful] [unhappy] [anxious] [intense]\n"
            ...
            f"[soft] [breathy] [whispering] [low voice] [volume up] [loud]\n\n"
            f"=== AUDIO EFFECTS / VOCALIZATIONS ===\n"
            f"[laughing] [chuckling] [sighing] [sobbing] [panting] [gasp] [inhale]\n"
            ...
            try:
                ...
                coro = self.context.llm_generate(
                    ...
                    system_prompt=(
                        ...
                        "Strictly adhere to the safety rules: "
                        "do not stack intense tags "
                        "(like [angry] and physical action tags) "
                        "to prevent TTS audio collapse. "
