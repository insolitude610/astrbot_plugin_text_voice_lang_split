...
class TextVoiceLangSplit(Star):
    ...
    async def _translate_text(self, text: str, event: AstrMessageEvent) -> str | None:
        ...
        prompt = (
            ...
            f"At the beginning of each sentence, insert ONE emotion tag "
            f"in English wrapped in square brackets. "
            f"Use English emotion words (NOT Japanese) — "
            f"English tags are much better supported by the TTS engine. "
            f"Prefer these tags: "
            f"[happy], [sad], [angry], [excited], [calm], [nervous], "
            f"[confident], [surprised], [satisfied], [delighted], "
            f"[scared], [worried], [upset], [frustrated], [depressed], "
            f"[empathetic], [embarrassed], [disgusted], [moved], "
            f"[proud], [relaxed], [grateful], [curious], [sarcastic], "
            f"[friendly], [helpful], [encouraging], [concerned]. "
            f"You may add intensity before the emotion word: "
            f"[slightly], [very], or [extremely] (e.g. [very happy]). "
            f"DO NOT put multiple emotions in one bracket. "
            f"If a sentence has multiple emotions, "
